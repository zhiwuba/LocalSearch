...
int Search_DB::search_range(key_t *left, const key_t &right,
...
int Search_DB::update(const key_t& key, value_t value)
{
...
void Search_DB::remove_from_index(off_t offset, internal_node_t &node,
								   const key_t &key)
{
...
bool Search_DB::borrow_key(bool from_right, internal_node_t &borrower,
							off_t offset)
{
...
bool Search_DB::borrow_key(bool from_right, leaf_node_t &borrower)
{
	off_t lender_off = from_right ? borrower.next : borrower.prev;
	leaf_node_t lender;
	map(&lender, lender_off);
...
void Search_DB::change_parent_child(off_t parent, const key_t &o,
									 const key_t &n)
{
...
void Search_DB::merge_leafs(leaf_node_t *left, leaf_node_t *right)
{
	std::copy(begin(*right), end(*right), end(*left));
	left->n += right->n;
}

void Search_DB::merge_keys(index_t *where,
...
void Search_DB::insert_record_no_split(leaf_node_t *leaf,
...
void Search_DB::insert_key_to_index(off_t offset, const key_t &key,
...
void Search_DB::insert_key_to_index_no_split(internal_node_t &node,
...
void Search_DB::reset_index_children_parent(index_t *begin, index_t *end,
											 off_t parent)
{
...
off_t Search_DB::search_index(const key_t &key) const
{
...
off_t Search_DB::search_leaf(off_t index, const key_t &key) const
{
	internal_node_t node;
	map(&node, index);
...
template<class T>
void Search_DB::node_create(off_t offset, T *node, T *next)
{
	// new sibling node
...
template<class T>
void Search_DB::node_remove(T *prev, T *node)
{
